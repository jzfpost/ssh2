...
class PhpSsh2
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (isset($options['dateFormat'])) {
            $this->screenLogging = $options['dateFormat'];
        }
    ...
    public function log($level, $message, array $context = array()): void
    {
        ...
        if ($level === 'none') {
            ...
            $text = '[' . date('Y M d H:i:s') . '] ' . $this->host . ' ' . $level . ': '
                . PHP_EOL
                . '----------------' . PHP_EOL
                . $message
                . PHP_EOL
            ...
            $text = '[' . date($this->dateFormat) . '] ' . $this->host . ' ' . $level . ': ' . $message . PHP_EOL;
        }
        if ($this->logging) {
     ...
     * @throws Ssh2Exception
     */
    public static function ignore_cb($message): void
    {
     ...
     * @throws Ssh2Exception
     */
    public static function macerror_cb($packet): void
    {
        throw new Ssh2Exception("MAC error: " . var_dump($packet));
    }
